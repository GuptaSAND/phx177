# This file is responsible for configuring your application
# and its dependencies with the aid of the Config module.
#
# This configuration file is loaded before any dependency and
# is restricted to this project.

# General application configuration
import Config

config :phx_177,
  ecto_repos: [Phx177.Repo]

# Configures the endpoint
config :phx_177, Phx177Web.Endpoint,
  url: [host: "localhost"],
  render_errors: [
    formats: [html: Phx177Web.ErrorHTML, json: Phx177Web.ErrorJSON],
    layout: false
  ],
  pubsub_server: Phx177.PubSub,
  live_view: [signing_salt: "9+JpGjcK"]

# Configures the mailer
#
# By default it uses the "Local" adapter which stores the emails
# locally. You can see the emails in your browser, at "/dev/mailbox".
#
# For production it's recommended to configure a different adapter
# at the `config/runtime.exs`.
config :phx_177, Phx177.Mailer, adapter: Swoosh.Adapters.Local

# Configure esbuild (the version is required)
# --target=esnext ->  latest features supported / default is es2017
config :esbuild,
  version: "0.17.11",
  dev: [
    args: ~w(js/app.js
    --analyze
    --log-level=debug
    --sourcemap=inline
    --bundle
    --target=es2017
    --external:/fonts/*
    --external:/images/*
    --outdir=../priv/static/assets),
    cd: Path.expand("../assets", __DIR__),
    env: %{"NODE_PATH" => Path.expand("../deps", __DIR__)}
  ],
  prod: [
    args: ~w(js/app.js
    --minify
    --analyze
    --sourcemap=linked
    --bundle
    --target=es2017
    --external:/fonts/*
    --external:/images/*
    --outdir=../priv/static/assets),
    cd: Path.expand("../assets", __DIR__),
    env: %{"NODE_PATH" => Path.expand("../deps", __DIR__)}
  ]

# Phoenix Assets Copy
config :phoenix_copy,
  images: [
    source: Path.expand("../assets/static/images/", __DIR__),
    destination: Path.expand("../priv/static/images/", __DIR__)
  ],
  docs: [
    source: Path.expand("../docs/", __DIR__),
    destination: Path.expand("../priv/static/docs/", __DIR__)
  ],
  default: [
    source: Path.expand("../assets/static/", __DIR__),
    destination: Path.expand("../priv/static/", __DIR__)
  ]

# This uses the tailwindcss options (basically a wrapper)

config :tailwind,
  version: "3.3.3",
  dev: [
    args: ~w(
      --config=tailwind.config.js
      --input=css/app.css
      --output=../priv/static/assets/app.css
    ),
    cd: Path.expand("../assets", __DIR__)
  ],
  prod: [
    args: ~w(
      --minify
      --config=tailwind.config.js
      --input=css/app.css
      --output=../priv/static/assets/app.css
    ),
    cd: Path.expand("../assets", __DIR__)
  ]

# Configures Elixir's Logger
config :logger, :console,
  format: "$time $metadata[$level] $message\n",
  metadata: [:request_id]

# Use Jason for JSON parsing in Phoenix
config :phoenix, :json_library, Jason

# Import environment specific config. This must remain at the bottom
# of this file so it overrides the configuration defined above.
import_config "#{config_env()}.exs"
